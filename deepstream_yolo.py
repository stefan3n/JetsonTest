...
def main():
    ...
    caps_display.set_property("caps", Gst.Caps.from_string("video/x-raw(memory:NVMM), format=I420"))
    ...
    if not videoconvert1.link(nvvidconv):
        ...
        sys.exit(1)
    if not nvvidconv.link(pgie):
        sys.stderr.write("Unable to link nvvidconv to pgie \n")
        sys.exit(1)
    ...
    if not nvosd.link(nvvidconv2):
        sys.stderr.write("Unable to link nvosd to nvvidconv2 \n")
        sys.exit(1)
    if not nvvidconv2.link(caps_display):
        sys.stderr.write("Unable to link nvvidconv2 to caps_display \n")
        sys.exit(1)
    if not caps_display.link(videoconvert2):
        sys.stderr.write("Unable to link caps_display to videoconvert2 \n")
        sys.exit(1)
